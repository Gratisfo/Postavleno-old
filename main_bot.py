...
from aiogram.types import InlineKeyboardMarkup, InlineKeyboardButton, CallbackQuery, ReplyKeyboardMarkup, \
    Message, ParseMode
from other import *
from config import api_token, admins_user_id
from texts import info_message, welcome_old
...
async def send_new_message(chat_id):
    ...
    text = data.get('start_user_message')
...
async def start(message: Message):
    ...
    keyboard = InlineKeyboardMarkup(row_width=1).add(button_1).add(button_2).add(button_3).add(button_4)

    logging.info(f"User {message.from_user.id} started the bot.")
    await message.answer(welcome_old, reply_markup=keyboard, parse_mode=ParseMode.HTML, disable_web_page_preview=True)

    data = get_json(json_file_path)
    start_status = data.get('start_status')
...
def add_new_user_to_db(message: Message):
    ...
    if user_id not in users_info:
        append_contact(user_id, first_name, last_name, username, 'new')
        print('append contact')
        db_new_user(user_id, username, first_name + ' ' + last_name)
        print('append contact to db')
    else:
        append_contact(user_id, first_name, last_name, username, 'old')
        print('append old contact')
...
async def admin_mode_start(message: Message):
...
async def add_news_text(message: Message):
...
async def process_size(message: Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        update_json('news_message', message.text)
...
async def add_item(message: Message):
    ...
    data = get_json(json_file_path)
    start_status = data.get('start_status')
    start_user_message = data.get('start_user_message')
    if start_user_message == '':
        ...
        await message.answer(f'Текст для отправки пользователям после запуска бота: \n\n{start_user_message}\n\n '
                             f'Статус: {start_status}', parse_mode=ParseMode.HTML, reply_markup=keyboard,
...
async def process_size(message: Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        data['text'] = message.text
        update_json('start_user_message', message.text)
    await message.answer(f"Сообщение для рассылки пользователям: \n\n{message.text}. \n\nВыберите действие\n\n"
                         f"Для отмены и перехода в главное меню администратора, вызовите команду /admin_mode",
                         parse_mode=ParseMode.HTML, reply_markup=keyboard, disable_web_page_preview=True)
    await state.finish()
...
async def add_item(message: types.Message):
    ...
    if post_text == '':
        ...
        await message.answer(f'Сообщение пользователям, которое приходит после таблицы: \n\n{post_text}\n\n '
                             f'Статус: {post_status}', parse_mode=ParseMode.HTML, reply_markup=keyboard,
                             disable_web_page_preview=True)
...
async def process_size(message: types.Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        data['text'] = message.text
        update_json('post_text', message.text)
    await message.answer(f"Сообщение для рассылки пользователям: \n\n{message.text}. \n\nВыберите действие\n\n"
                         f"Для отмены и перехода в главное меню администратора, вызовите команду /admin_mode",
                         parse_mode=ParseMode.HTML, reply_markup=keyboard, disable_web_page_preview=True)
    await state.finish()
...
@dp.callback_query_handler(text='send_news')
async def admin_message_send(callback_query: CallbackQuery):
    data = get_json(json_file_path)
    text = data.get('news_message')
    ...
    users_with_null_status = cursor.fetchall()
    for user_id in users_with_null_status:
        user_id = int(user_id[0])
        try:
            await bot.send_message(user_id, text=text, parse_mode=ParseMode.HTML, disable_web_page_preview=True)
            logging.info(f"Пользователю {user_id} отправлено news сообщение")
            await asyncio.sleep(1)
            cursor.execute('''
                        UPDATE users_data
                        SET status = ?, last_news = ?
                        WHERE id = ?
                    ''', (True, formatted_date, user_id))
            conn.commit()

        except:
            logging.error(f"News не отправлено: {user_id}")
            cursor.execute('''
                        UPDATE users_data
                        SET status = ?, last_news = ?
                        WHERE id = ?
                    ''', (False, formatted_date, user_id))
            conn.commit()
...
async def callback_query_keyboard(callback_query: CallbackQuery):
    if callback_query.data == 'edit_news':
        ...
        update_json('start_status', 'Активно')
        data = get_json(json_file_path)
        start_status = data.get('start_status')
        ...
        update_json('start_status', 'Неактивно')
        data = get_json(json_file_path)
        start_status = data.get('start_status')
        await callback_query.message.answer(f'Обновленный статус: {start_status} \n\n'
                                            'Для перехода в главное меню администратора, '
        ...
        update_json('post_status', 'Активно')
        data = get_json(json_file_path)
        post_status = data.get('post_status')
        ...
        update_json('post_status', 'Неактивно')
        data = get_json(json_file_path)
        post_status = data.get('post_status')
        await callback_query.message.answer(f'Обновленный статус: {post_status} \n\n'
                                            'Для перехода в главное меню администратора, '
    ...
    # Замена склада
        ...
        await callback_query.answer()
        print("начали обработку изменения склада")
        ...
        warehouse_count, deleted_warehouses, last_message_time = get_user_data(user_id)
        print('получили данные из базы по смене', callback_query.data)
